...
document.getElementById('close').addEventListener('click', closeSection);
...
document
  .querySelector('.currency-form input[type=submit]')
  .addEventListener('click', currencyExchange);

function currencyExchange(e) {
  e.preventDefault();
  const xhr = new XMLHttpRequest();
  const currencyFrom = document.querySelector(
    '.currency-form select[name=currencyFrom]'
  ).value;
  const currencyTo = document.querySelector(
    '.currency-form select[name=currencyTo]'
  ).value;
  const quantity = document.querySelector('.currency-form input[name=quantity]')
    .value;
...
function buidCurrencyList(lst) {
  const slctFrom = document.querySelector(
    '#extra #currencyConverter .part select[name=currencyFrom]'
  );
  const slctTo = document.querySelector(
    '#extra #currencyConverter .part select[name=currencyTo]'
  );
  for (key in lst) {
